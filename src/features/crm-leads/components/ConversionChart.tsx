 ...
 */
export function ConversionChart({ metrics, stages, dateRange }: ConversionChartProps) {
  return (
    <Stack gap="lg">
...
function MetricCard({ title, value, icon, color, description }: MetricCardProps) {
  return (
    <Paper p="md" withBorder data-testid={`metric-card-${title.toLowerCase().replace(/\s+/g, "-")}`}>
      <Stack gap="xs">
        ...
        <Text size="xl" fw={700} data-testid={`metric-value-${title.toLowerCase().replace(/\s+/g, "-")}`}>
          {value}
        </Text>
...
function StageVelocityChart({ stageVelocity }: StageVelocityChartProps) {
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder data-testid="stage-velocity-chart">
      <Stack gap="md">
        <Group gap="xs">
...
function calculateAverageVelocity(stageVelocity: Record<string, number>): string {
  ...
  if (velocities.length === 0) return "0 days";
