...
export function CreateSubscriptionModal({
  ...
  const form = useForm<{
    ...
    validate: {
      trialDays: (value) =>
        value !== undefined && value < 0 ? t`Trial days must be positive` : null,
    },
  });
  ...
  const handleSubmit = (values: typeof form.values) => {
    ...
    createSubscription.mutate(request, {
      ...
      onError: (error: any) => {
        notifications.show({
          title: t`Error`,
          message: error.message || t`Failed to create subscription. Please try again.`,
          color: "red",
        });
      },
    });
  ...
  return (
    <Modal opened={opened} onClose={onClose} title={t`Create Subscription`} size="md">
