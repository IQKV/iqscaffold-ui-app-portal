...
export const UsageBasedBillingProjection: React.FC<
  UsageBasedBillingProjectionProps
> = ({ billingProjection, loading = false }) => {
  ...
  const costIncrease = getProjectionPercentage(
    billingProjection.currentCost,
    billingProjection.projectedCost
  );
    ...
    <Card withBorder radius="md" p="lg">
      <Stack gap="md">
        ...
        <Grid>
          ...
          <Grid.Col span={{ base: 12, sm: 4 }}>
            <Card
              ...
              p="md"
              radius="sm"
              bg={`${getProjectionColor(billingProjection.currentCost, billingProjection.projectedCost)}.0`}
            >
              <Stack gap="xs" align="center">
                <Text size="xs" c="dimmed" ta="center">
                  Projected Cost
        ...
        <Box>
          ...
          <Stack gap="sm">
            {Object.entries(billingProjection.currentPeriodUsage).map(
              ([metric, current]) => {
                const projected =
                  billingProjection.projectedUsage[metric] || current;
                const percentage = getProjectionPercentage(current, projected);
                const progressValue = Math.min(
                  (current / projected) * 100,
                  100
                );
        ...
        <Stack gap="sm">
            ...
            >
              <Group justify="space-between" align="center">
                <Box>
                  ...
                  <Text size="sm">
                    Your projected usage will result in{" "}
                    {formatCurrency(billingProjection.overageCost)} in overage
                    charges.
                  </Text>
                </Box>
            ...
            >
              <Group justify="space-between" align="center">
                <Box>
                  ...
                  <Text size="sm">
                    You could save{" "}
                    {formatCurrency(billingProjection.potentialSavings)} by
                    optimizing your usage or plan.
                  </Text>
                </Box>
                <Button
                  variant="light"
          ...
          {!hasOverage && !hasSavings && (
            ...
            >
              <Text size="sm">
                Your usage is well within your plan limits. Your projected cost
                is {formatCurrency(billingProjection.projectedCost)} for this
                billing period.
              </Text>
            </Alert>
          )}
